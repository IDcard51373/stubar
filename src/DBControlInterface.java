...
public interface DBControlInterface {
    ...
    //加载主页面。
    //导出所有帖子中最新刷新的5个帖子的Post_id、Title和Content的前50个字符，存在一个10*2的数组中。
    String[][] DB_GetMainPage();

    //加载某板块页面。
    //导入专业板块的名称。
    //导出该板块最近刷新的5个帖子的Post_id、Title和Content的前50个字符，存在一个10*2的数组中。
    ...
    //加载某帖子的评论。
    //导入该帖子的Post_id。
    //导出一条最新（不最新也行）评论的Comment_id，Time，Content，评论发布者的User_name。
